...
between packages within one `packages:` list.

Naming a group that is not one of this specification's own
`multiconfig:` keys, naming a group itself, or a circle of groups naming
each other, is a parse-time error.
...
`extends: uki: initrd:` is deliberately lenient about *when* its named
`initrd:` artifact has to exist: a group with no declared predecessor
still has this checked as soon as the specification is parsed, the same
fail-fast behaviour `extends: kernel`/`module` get;
