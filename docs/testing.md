...
(a fragment reached via `requires` loses a field both files set; a
second file named directly on the command line amends instead), and
for why `keywords:`/`steps:` are refused rather than merged on a real
mismatch either way. It is not a second,
