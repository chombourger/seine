...
class Imager:
    ...
    def _normalize_ext_mount(self, g, m, mounts, mount_devices):
        ...
        # Only an ext2/3/4 child has that same concern (and, if one, gets
        # its own read-only mount on its own turn) -- anything else (FAT,
        # a still-staged RO_FSTYPES mount) goes back read-write, since
        # later steps (UKI anchoring, RO image building) still write to it.
    ...
    def _build_ro_images(self, g, mounts, mount_devices, part_index, hash_part_for):
        ...
        # A rebuilt ext2/3/4 root is remounted read-only by then
        # (_normalize_ext_mount, for reproducibility) -- use the scratch
        # disk instead of writing to it. Mounted on '/mnt': a normal
        # FHS directory, unlike SCRATCH_MOUNT it survives root's rebuild
        # (which only keeps what's actually part of the rootfs content).
