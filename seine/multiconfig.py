...
#
# 'defer_uki_check' skips 'extends: uki: initrd:'s parse-time check that
# the named 'initrd:' is already deployed -- set for a group declared
# 'after:' another whose build (in this same run) will deploy it.
...
# One group's declared value: a bare file list (no ordering), or
# {files:, after:, before:} naming other groups to build after/before --
# the same relationship 'packages: after:'/'before:' has between
# packages. Returns (files, after, before); resolve_order() below folds
# both into one 'after' set per group.
