...
class GrubBootloader(Bootloader):
    ...
    # A bare call (today's only case) keeps update-grub's own
    # auto-discovery. A real 'root_partuuid' means a 'multiconfig:' group
    # instead: found at boot by filesystem label, not PARTUUID -- this
    # grub's 'search' has no '--part-uuid' option (checked against the
    # actual trixie/bookworm grub-efi-amd64-bin, only '--label'/
    # '--fs-uuid'/'--file'). The kernel command line still gets
    # 'root=PARTUUID=...', which the kernel resolves natively.
    def add_entry(self, g, group_label=None, kernel=None, initrd=None,
