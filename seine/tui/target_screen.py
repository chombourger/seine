...
# driven through mtda (target.py). No spec tree here, unlike every
# other screen. Width split 3:2 (console : status).
#
# Two ways to type at the target: a line at the normal Prompt (a
# non-'/', non-'!' line goes to console_send() instead of AI chat), or
# focusing ConsolePane for raw keystrokes (arrows, Ctrl combos, Esc --
# e.g. to catch a PC's "press Esc for BIOS setup" window, which a
# line-buffered input can never do in time).
#
# No confirmation for entering raw mode: confirming every keystroke
# would be unusable, and focusing the pane is already deliberate. Just
# a transient status-line reminder that keystrokes now go to the target.
...
# Same braille spinner chat.py's '#chatcol' border uses while working.
FRAMES = SPINNER[True]

# VT100/ANSI byte sequences for keys that don't already produce a
# literal character. 'tab' is deliberately not here: Tab already means
# "switch pane" app-wide, and overriding that in raw mode would surprise.
_KEY_BYTES = {
...
# Reads the 'target-click' marker render_target_status() puts in a
# span's meta: a plain value, not Rich's '@click' action-link string,
# which Textual overlays with its own link style regardless.
...
# Unlike every other StaticPane: this one is the reason to be on this
# screen, so it takes a Tab stop like the prompt does.
class ConsolePane(StaticPane):
    ...
    # Enabled immediately, no confirm: a persistent drain worker for as
    # long as the pane stays focused. One thread only, so keystrokes
    # stay in order.
...
class TargetScreen(BaseScreen):
    ...
    /* min-width: CONSOLE_COLUMNS (80) plus border chrome: Textual
       doesn't auto-shrink a fixed-width child, and this firmware needs
       all 80 columns. No min-height: #console's height is capped, not
       fixed, and _redraw_console() only asks for rows that fit. */
    #console-pane {
    ...
    # Set here, not on_mount(): '/target connect' can call this
    # screen's connect() right after app.show("target") constructs it,
    # reaching _update_console_border() before on_mount() runs.
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def on_mount(self):
        super().on_mount()
        # Status pane + console pane's uptime/spinner border: a 1s tick,
        # not a push per POWER/STORAGE event (rare state transitions).
        self._status_timer = self.set_interval(1.0, self._redraw_status)
        # Console pane: a bounded-rate poll of ConsoleAdapter.dirty, not
        # a push per incoming chunk -- see ConsoleAdapter for why
        # push-per-chunk made a boot log look like it typed one
        # character at a time. ~30/s reads as live for interactive
        # typing while capping a boot log's redraw rate.
        self._console_timer = self.set_interval(1 / 30, self._maybe_redraw_console)
        # No auto-connect otherwise: '/target connect [agent]' is how a
        # first connection happens; this only re-dials when there's
        # already a live client to refresh, or $MTDA_REMOTE names a
        # default agent, same as mtda-cli.
    ...
    # force=True is '/target connect [agent]': always a real
    # teardown-and-redial, even with no host. force=False is
    # on_mount()'s refresh: dial only if nothing is connected yet.
    ...
    def _connect(self, host=None, force=False):
        ...
        # and this is often the first RPC to actually reach the host.
        # A failure here rolls the connection back rather than leaving
        # it half-open.
        try:
    ...
    # Same transient #status.warning ConsolePane.on_focus() uses: dump()
    # is a snapshot, not a full redraw, so a full-screen app can look
    # partial/stale until it next repaints itself.
    ...
    # Runs on the UI thread already (Textual's set_interval); only the
    # worker-thread paths cross that boundary with call_from_thread.
    ...
    # upstream to catch a bad frame (e.g. a colour Rich's Style()
    # rejects), which would otherwise hit Textual's fatal-error handler.
    def _redraw_console(self):
        ...
        try:
            if adapter is None:
                ...
                # -2 for #console-pane's border: how many of the pyte
                # screen's rows actually fit right now, not the fixed
                # CONSOLE_LINES ceiling.
    ...
    # '#console-pane' spends its border on the target's uptime while
    # it's on, gone the moment power_on_at clears (target off, or not
    # connected), same as '#chatcol' clears its 'Working…' subtitle.
    ...
    # '/command' and '!shell' still go into normal persisted history;
    # only a genuine console line is diverted to the in-memory-only
    # 'target' record set, so a password typed at a login prompt never
    # reaches history.json.
    ...
    # escape they look like, since a single-line Input can't send a
    # literal control byte otherwise.
